...
/**
 * Create and [remember] the [CustomScrollState] based on the currently appropriate scroll
 ...
 */
@Composable
fun rememberCustomScrollState(initial: Float = 0F): CustomScrollState {
    return rememberSaveable(saver = CustomScrollState.Saver) {
        CustomScrollState(initial = initial)
    }
}
 ...
 *
 * To create and automatically remember [CustomScrollState] with default parameters use
 * [rememberCustomScrollState].
 *
 * Learn how to control the state of [Modifier.verticalScroll] or [Modifier.horizontalScroll]:
 * @sample androidx.compose.foundation.samples.ControlledScrollableRowSample
 *
 ...
 */
@Stable
class CustomScrollState(initial: Float) : ScrollableState {
    ...
    private var _maxValueState = mutableStateOf(Float.MAX_VALUE, structuralEqualityPolicy())
    ...
    suspend fun animateScrollTo(
        ...
        this.animateScrollBy((value - this.value), animationSpec)
    }
     ...
     */
    suspend fun scrollTo(value: Int): Float = this.scrollBy((value - this.value))

    companion object {
        /**
         * The default [Saver] implementation for [CustomScrollState].
         */
        val Saver: Saver<CustomScrollState, *> = Saver(
            save = { it.value },
            restore = { CustomScrollState(it) }
        )
    }
}
 ...
 *
 * In order to use this modifier, you need to create and own [CustomScrollState]
 * @see [rememberCustomScrollState]
 *
 * @param stateCustom state of the scroll
 * @param enabled whether or not scrolling via touch input is enabled
 * @param flingBehavior logic describing fling behavior when drag has finished with velocity. If
 * `null`, default from [ScrollableDefaults.flingBehavior] will be used.
 * @param reverseScrolling reverse the direction of scrolling, when `true`, 0 [CustomScrollState.value]
 * will mean bottom, when `false`, 0 [CustomScrollState.value] will mean top
 */
fun Modifier.verticalScroll(
    stateCustom: CustomScrollState,
 ...
 * `null`, default from [ScrollableDefaults.flingBehavior] will be used.
 * @param reverseScrolling reverse the direction of scrolling, when `true`, 0 [CustomScrollState.value]
 * will mean right, when `false`, 0 [CustomScrollState.value] will mean left
 */
    ...
    factory = {
        ...
        val semantics = Modifier.semantics {
            val accessibilityScrollState = ScrollAxisRange(
                value = { stateCustom.value },
                maxValue = { stateCustom.maxValue },
                reverseScrolling = reverseScrolling
            )
            if (isVertical) {
                ...
                scrollBy(
                    action = { x: Float, y: Float ->
                        coroutineScope.launch {
                            if (isVertical) {
                                (stateCustom as ScrollableState).animateScrollBy(y)
                            } else {
                                (stateCustom as ScrollableState).animateScrollBy(x)
                            }
                        }
        ...
        val scrolling = Modifier.scrollable(
            ...
            interactionSource = stateCustom.internalInteractionSource,
            flingBehavior = flingBehavior,
            state = stateCustom
        )
        val layout =
            ScrollingLayoutModifier(stateCustom, reverseScrolling, isVertical)
        semantics
...
private data class ScrollingLayoutModifier(
    val scrollerStateCustom: CustomScrollState,
    ...
    override fun MeasureScope.measure(
        ...
        val side = if (isVertical) scrollHeight.toFloat() else scrollWidth.toFloat()
        return layout(width, height) {
            scrollerStateCustom.maxValue = side
            val scroll = scrollerStateCustom.value.coerceIn(0F, side)
            val absScroll = (if (isReversed) scroll - side else -scroll).toInt()
